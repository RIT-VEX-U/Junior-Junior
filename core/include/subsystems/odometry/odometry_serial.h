...
class OdometrySerial : public OdometryBase {
  public:
    ...
    void set_position(Pose2d &new_pose);
    void calibrate_imu(uint8_t &samples);
    void reset_tracking();
    void set_linear_scalar(const float &linear_scalar);
    void set_angular_scalar(const float &angular_scalar);
    void set_offset(Pose2d &sensor_offset);

    void request_pos(Pose2d &pos);
    void request_vel(Pose2d &vel);
    void request_acc(Pose2d &acc);


    void request_pos_vel(Pose2d &pos, Pose2d &vel);
    void request_pos_acc(Pose2d &pos, Pose2d &acc);
    void request_vel_acc(Pose2d &vel, Pose2d &acc);

    void request_pos_vel_acc(Pose2d &pos, Pose2d &vel, Pose2d &acc);

    int send_packet(uint32_t port, uint8_t *buffer, size_t buffer_size);
